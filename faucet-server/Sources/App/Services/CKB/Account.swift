...
//    var publicKeyHash: H256 {
//        let hash = Blake2b().hash(data: Data(hex: publicKey))!.toHexString()
//        return Utils.prefixHex(Blake2b().hash(data: Data(hex: hash))!.toHexString())
//    }
    var lock: Script {
        return Script.verifyScript(for: publicKey, binaryHash: systemScriptCellHash)
        //        return Script(args: [publicKeyHash], binaryHash: systemScriptCellHash)
    }
    var lockHash: H256 {
...
//        let systemScriptCell = try api.genesisBlock().commitTransactions[0]
...
//        systemScriptOutPoint = OutPoint(hash: systemScriptCell.hash, index: 0)
